...
impl TemperatureSampler {
    pub fn new(temperature: f32) -> Result<Self> {
        Self::with_options(temperature, None, None)
    }

    pub fn with_seed(temperature: f32, seed: u64) -> Result<Self> {
        Self::with_options(temperature, Some(seed), None)
    }

    pub fn with_options(temperature: f32, seed: Option<u64>, top_k: Option<usize>) -> Result<Self> {
...
impl Sampler for TemperatureSampler {
    fn sample(&mut self, logits: &crate::tensor::Tensor) -> Result<usize> {
        ...
        let prob = softmax(&candidate_logits)?;
...
mod tests {
    ...
    fn temperature_sampler_rejects_zero_top_k() {
        expect_invalid_top_k(TemperatureSampler::with_options(1.0, None, Some(0)), 0);
    }
    ...
    fn top_k_one_selects_highest_logit_token() {
        ...
        let mut sampler =
            TemperatureSampler::with_options(1.0, Some(42), Some(1)).expect("valid sampler");
        let token_id = sampler.sample(&logits).expect("sample should succeed");
    ...
    fn top_k_larger_than_vocab_uses_full_vocab() {
        ...
        let mut sampler =
            TemperatureSampler::with_options(1.0, Some(42), Some(10)).expect("valid sampler");
        let token_id = sampler.sample(&logits).expect("sample should succeed");
